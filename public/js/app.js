...
$.ajax({
  type: 'PUT',
  url: 'Associar_Usuario/' + idAtividade + '/' + idUsuario,
  data: formData, 
  processData: false,
  contentType: false, 
  headers: {
...
if (atividade && atividade.hasOwnProperty('usuarios') && Array.isArray(atividade.usuarios)) {
  ...
  $('#formComentario').on('submit', function (event) {
    ...
    $.ajax({
      type: 'POST',
      url: 'Comentarios/Criar_Comentarios/' + atividadeId + '/' + usuarioId,
      data: formData, 
      processData: false,
      contentType: false, 
      headers: {
...
console.log(atividade);
  ...
  $('#atividadeForm').on('submit', function (event) {
    ...
    debugger;
    $.ajax({
      url: 'Atualizar_Atividade/' + idAtividade,
      type: 'POST', 
      data: formData,
      processData: false,
      contentType: false, 
...
$(document).ready(function () {
  $('#btnDeletarAtividade').on('click', function () {
    ...
    $.ajax({
      url: 'Deletar_Atividade/' + idAtividade,
      type: 'DELETE',
      headers: {
        'X-CSRF-TOKEN': csrfToken
      },
...
$('#Delete_card_Button').on('click', function(){
  ...
  $.ajax({
    url: '/Card/Atividade/Deletar/' + idCard,
    type: 'DELETE',
    headers: {
      'X-CSRF-TOKEN': csrfToken
    },
