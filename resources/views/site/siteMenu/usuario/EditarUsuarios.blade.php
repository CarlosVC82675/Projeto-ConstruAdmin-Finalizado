...
@section('conteudo')
   ...
   <form action="{{ route('usuarios.atualizar', ['id' => $usuario->idUsuario])}}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="mb-3">
            ...
            <select class="form-select" aria-label="Default select example" name="atribuicao" readonly>
                <option value="{{$usuario->getRoleNames()}}" selected>{{$usuario->getRoleNames()}}</option>
              </select>
        </div>
        <div class="mb-3">
